...
@patch('adapters.gersbach_E2G_perturb_seq_adapter.FileFileSet')
@patch('adapters.gersbach_E2G_perturb_seq_adapter.GeneValidator')
def test_gersbach_e2g_perturb_seq_adapter_genomic_elements(mock_gene_validator, mock_file_fileset):
    # Mock the FileFileSet to avoid external API calls
    mock_file_instance = mock_file_fileset.return_value
    mock_file_instance.query_fileset_files_props_igvf.return_value = [
        {'method': 'Perturb-seq'}]

    mock_validator_instance = mock_gene_validator.return_value
...
@patch('adapters.gersbach_E2G_perturb_seq_adapter.FileFileSet')
@patch('adapters.gersbach_E2G_perturb_seq_adapter.GeneValidator')
def test_gersbach_e2g_perturb_seq_adapter_genomic_elements_genes(mock_gene_validator, mock_file_fileset):
    # Mock the FileFileSet to avoid external API calls
    mock_file_instance = mock_file_fileset.return_value
    mock_file_instance.query_fileset_files_props_igvf.return_value = [
        {'method': 'Perturb-seq'}]

...
class Clickhouse:
    ...
    def __init__(self):
        ...
        if Clickhouse.__connection is None:
    ...
    def process_json(self, json_line, properties):
        ...
        if data['_from'] is not None and data['_to'] is not None:
    ...
    def import_file(self, original_jsonl_filepath, collection):
        ...
        sql_properties += ',' + schema.get('relationship', {}).get('from', '')
        sql_properties += ',' + schema.get('relationship', {}).get('to', '')

        print('Loading data...')

        bulk = []
        with open(original_jsonl_filepath, 'r') as jsonl:
            for json_line in jsonl:
                bulk.append(tuple(self.process_json(json_line, properties)))

                if len(bulk) == 10000:
                    try:
                        client.execute(f'INSERT INTO {collection} ({
    ...
    def generate_sql_schema(self, output_filepath):
        ...
        with open(output_filepath, 'w') as clickhouse_schema:
            ...
            clickhouse_schema.write(
                f'SET allow_experimental_json_type = 1;\n\n')

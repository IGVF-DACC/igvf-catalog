...
def add_key_spdi_hgvs(input_file, output_file):
    ...
    validator = Draft202012Validator(
        get_schema('nodes', 'variants', 'autogenerated_topld'))
    ...
    for line in open(input_file, 'r'):
        ...
        data['vrs_digest'] = allele_vrs_digest

        try:
            validator.validate(data)
        except ValidationError as exc:
            print('Schema validation failed for %s: %s' %
                  (data['_key'], exc.message))
            total_invalid += 1
            continue
    ...
    print('Done. Written: %d | Skipped (invalid): %d -> %s' %
          (total_written, total_invalid, output_file))
...
def process_chr(chr):
    ...
    output = open(
        'autogenerated_topld_variants_chrC.jsonl'.replace('C', chr), 'w+')
        ...
        for line in open(topld_path, 'r'):
            ...
            if data['_to'].split('variants/')[1] not in favor_keys:
                if data['variant_2_rsid'].startswith('chr'):
                    variant = create_variant(data['variant_2_rsid'])
                    output.write(json.dumps(variant) + '\n')

...
def query_fileset_files_props_encode(file_accession, source_url, prediction=True, additional_props=None):
    ...
    allowed_additional_props = [
        'donor', 'treatment', 'pmid', 'software']  # expand
    ...
    # treatments info e.g. ENCSR881BOY -> /treatments/796eabb9-70be-4412-bc63-cb4329a9239b/
    treatments_term_ids = []
    # from notes? e.g. 1 h with 100 nM Dexamethasone (Myers)
    treatments_info = []
    if fileset_json['treatments']:
        for treatment in fileset_json['treatments']:
            treatments_term_ids.append(
                treatment['treatment_term_id'])  # e.g. CHEBI:41879
            # need an example from protein
            treatments_info.append(treatment['notes'])

    _id = file_accession
    props = {
        '_key': _id,
        'fileset_id': fileset_json['accession'],
        # sample -> NTR?
        # e.g. "/human-donors/ENCDO000AAD/"
        'donor': fileset_json['donor'] if fileset_json['donor'] else None,
        # reformat to ontology_terms/...
        'treatments_term_ids': treatments_term_ids if treatments_term_ids else None,

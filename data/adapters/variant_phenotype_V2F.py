...
# 1	13549   G   C	rs554008981	0	0.503440541028976	0.504938441514969	0.412174689769745	0.508037388324738	0.458340626955032	0.572471630573273	0.659147709608078
# 1	14463   A   T	rs546169444	0	0.302759596705437	0.490839466452599	0.522221061587334	0.58622761964798	0.425303816795349	0.445275992155075	0.399437755346298
...
class VariantPhenotypeV2FAdapter:
    ...
    def process_file(self):
        ...
        with open(self.filepath, 'r') as cV2F_file:
            cV2F_csv = csv.reader(cV2F_file, delimiter='\t')
            next(cV2F_csv)
            for row in cV2F_csv:
                chr = row[0]
                pos = row[1]
                ...
                _props = {
                    ...
                    'linkage_disequilibrium': float(row[5]),
                    'general_context_score': float(row[6]),
                    'liver_context_score': float(row[7]),
                    'blood_context_score': float(row[8]),
                    'brain_context_score': float(row[9]),
                    'GM12878_context_score': float(row[10]),
                    'K562_context_score': float(row[11]),
                    'HepG2_context_score': float(row[12]),

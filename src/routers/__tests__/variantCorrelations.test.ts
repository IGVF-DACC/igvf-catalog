import {
  getVariantCorrelations,
  variantCorrelationFormat,
  variantCorrelationQueryFormat,
  variantCorrelations,
} from "../variantCorrelations";
import { z } from "zod";
...
describe("Snp Correlation Endpoint", () => {
  ...
  test("Receives as input a snpCorrelationQueryFormat object", () => {
    ...
    expect(variantCorrelations._def.inputs[0]).toBe(
      variantCorrelationQueryFormat
    );
  });
  ...
  test("Outputs an array of snpCorrelations", () => {
    ...
    expect(outputFormat._def).toMatchObject(
      z.array(variantCorrelationFormat)._def
    );
  });
  ...
  describe("getSnpCorrelations", () => {
    ...
    test("queries all correlations", async () => {
      ...
      const snpCorrelations = await getVariantCorrelations(
        rsid,
        ancestry,
        queryPage
      );
      ...
      expect(mockQueryDb).toHaveBeenCalledWith(
        expect.objectContaining({ bindVars })
      );

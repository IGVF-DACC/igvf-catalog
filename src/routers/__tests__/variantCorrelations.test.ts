...
describe('variant Correlation Endpoint', () => {
  beforeEach(() => {
    jest.resetModules()
  })
  ...
  describe('OpenAPI', () => {
    ...
    test('has correct URL', () => {
      ...
      expect(openApi?.path).toBe('/variant-correlations/{chr}')
    })
  })

  test('Receives as input a variantCorrelationQueryFormat object', () => {
  ...
  test('Outputs an array of variantCorrelations', () => {
  ...
  describe('getvariantCorrelations', () => {
    ...
    test('queries all correlations', async () => {
      ...
      const variantCorrelations = await getvariantCorrelations(chr, r, queryPage)
      ...
      expect(variantCorrelations).toMatchObject([variantCorr])
    ...
    test('undefined or null page value defaults to page 0', async () => {
      ...
      await getvariantCorrelations(chr, r, undefined)

      const bindVars = { '@value0': collectionName, value1: r, value2: parseInt(chr), value3: queryPage, value4: 100 }

      expect(mockQueryDb).toHaveBeenCalledWith(expect.objectContaining({ bindVars }))

      await getvariantCorrelations(chr, r, null)

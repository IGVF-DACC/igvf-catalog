...
describe("routerTransitiveClosure", () => {
  ...
  describe("constructor", () => {
    test("it parses config fields accordingly", () => {
      ...
      const router = new RouterTransitiveClosure(
        schemaConfig["variant to variant correlation"]
      );
  ...
  describe("getPaths", () => {
    ...
    test("queries correct DB collection and return matched records", async () => {
      ...
      router = new RouterTransitiveClosure(
        schemaConfig["variant to variant correlation"]
      );
      const records = await router.getPaths("vertex_1", "vertex_3");

      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining(`${router.edgeDBCollectionName}`)
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("FOR path IN OUTBOUND ALL_SHORTEST_PATHS")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("'vertex_1' TO 'vertex_3'")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("RETURN path")
      );
    ...
    test("decodes search term before querying", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("IN OUTBOUND ALL_SHORTEST_PATHS")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("'brain:GO_0070257' TO 'spine:GO_0070258'")
      );
    });
  });
  ...
  describe("generateRouter", () => {
    ...
    beforeEach(() => {
      ...
      routerBuilder = new RouterTransitiveClosure(
        schemaConfig["variant to variant correlation"]
      );

  ...
  describe("constructor", () => {
    test("it parses config fields accordingly", () => {
      ...
      expect(router.dbReturnStatements).toEqual(
        "_id: record._key, 'uri': record['uri'], 'label': record['label']"
      );
    });
  });
  ...
  describe("getObjectByFuzzyTextSearch", () => {
    ...
    test("queries correct DB collection and return matched records", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining(`IN ${router.searchViewName()}`)
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("SEARCH LEVENSHTEIN_MATCH")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("record.label")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('TOKENS("brain", "text_en_no_stem")')
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining(`LIMIT ${page}, ${QUERY_LIMIT}`)
      );
    ...
    test("decodes search term before querying", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('TOKENS("brain:GO_0070257", "text_en_no_stem")')
      );
    });
  });

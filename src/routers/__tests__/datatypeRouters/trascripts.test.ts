...
describe('genes routers', () => {
  ...
  describe('it implements exact match by ID', () => {
    ...
    beforeEach(() => {
      router = transcriptsRouters.transcriptID
      openApi = router._def.meta?.openapi
    })
    ...
    test('accepts transcript by ID format', () => {
      const genesByIDQuery = {
        id: 'ENST00000353224'
      }

      const inputParsing = router._def.inputs[0].parse(genesByIDQuery)
      expect(inputParsing).toEqual(genesByIDQuery)
    })
    ...
    test('outputs a single variant in correct format', () => {
      ...
      const inputParsing = router._def.output.parse(gene)
      expect(inputParsing).toEqual(gene)
    })

    test('has correct URL', () => {
      expect(openApi?.method).toBe('GET')
      expect(openApi?.path).toBe('/transcripts/{id}')
    })

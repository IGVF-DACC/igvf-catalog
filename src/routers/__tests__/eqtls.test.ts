  ...
  describe("getQtls", () => {
    test("queries correct DB collection and return eqtls", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("IN variant_gene_links")
      );
    ...
    test("returns 400 if neither gene id nor biological context nor coordinates range are provided", async () => {
      try {
        ...
        expect(message).toBe(
          "Either a gene ID or a biological context or coordinate range must be provided."
        );
        return;
      }
      fail(
        "Either gene ID or biological context or coordinate range must be provided."
      );
    });
    ...
    test("filters by gene ID, biological context, coordRange, pvalue, and beta", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("link._to == 'genes/gene_12345'")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("link.biological_context == 'brain'")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining(
          "variant.chr == 'chrY' and variant['pos:long'] >= 2781418 and variant['pos:long'] <= 2781520"
        )
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("link['p-value:long'] <= 0.5")
      );
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("link['beta:long'] <= 0.1")
      );
    });
    ...
    test("fails with invalid coordinate range format", async () => {
      try {
        ...
        const message = (e as TRPCError).message;
        expect(message).toBe(
          "coordRange parameter expects the format: 'chr:from:to'. For example: chrY:2781418:2781520"
        );
        return;
      }
    ...
    test("paginates with default page size of 25 and deafults to page 0", async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("LIMIT 3, 25")
      );

      await getQtls("gene_12345", "brain", 0.5, 0.1, null, null);
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining("LIMIT 0, 25")
      );
    });
  });
});

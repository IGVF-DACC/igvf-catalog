...
export function getActiveNodes(
  schemaConfig: Record<string, configType>
): Set<string> {
...
export function getActiveEdges(
  ...
  Object.keys(schemaConfig).forEach((schema) => {
    if (Object.hasOwn(schemaConfig[schema], "relationship")) {
      const schemaRelationships = schemaConfig[schema].relationship as Record<
        string,
        string
      >;

      if (
        nodes.has(schemaRelationships.to) &&
        nodes.has(schemaRelationships.from)
      ) {
        edges.add(schema);
      }
    }
  ...
  Object.keys(schemaConfig).forEach((schema) => {
    if (Object.hasOwn(schemaConfig[schema], "relationship")) {
      const schemaRelationships = schemaConfig[schema].relationship as Record<
        ...
        string
      >;
      const edgeDBCollectionName = schemaConfig[schema]
        .db_collection_name as string;
  ...
  Object.keys(schemaConfig).forEach((schema) => {
    ...
    if (Object.hasOwn(schemaObj, "accessible_via")) {
      ...
      if (relationships.children.length > 0) {
        ...
        routers[router.apiName + "_children"] =
          router.generateRouter("children");
      }

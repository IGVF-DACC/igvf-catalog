...
const genesProteinsGoRouter = new RouterEdges(goTermsAnnotationsSchema)
...
async function goTermsSearch (input: paramsFormatType): Promise<any[]> {
  ...
  if (annotations.length > 0) {
    response = await genesProteinsGoRouter.getSourceAndEdgeSet(annotations, customSourceFields, customTargetFields, 'transcripts', page)
...
async function geneProteinSearch (input: paramsFormatType): Promise<any[]> {
  ...
  return await genesProteinsGoRouter.getTargetAndEdgeSet(id, customSourceFields, customTargetFields, page)
}

const goTermsFromAnnotations = publicProcedure
  .meta({ openapi: { method: 'GET', path: '/annotations/go-terms', description: descriptions.genes_proteins_go_terms } })
  .input(queryFormat)
  .output(z.array(goAnnotationFormat))
  .query(async ({ input }) => await goTermsSearch(input))

const annotationsFromGoTerms = publicProcedure
  .meta({ openapi: { method: 'GET', path: '/go-terms/annotations', description: descriptions.go_terms_genes_proteins } })

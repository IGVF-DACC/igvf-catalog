...
const phenotypesFromVariants = publicProcedure
  ...
  .input(variantsQueryFormat.omit({ funseq_description: true }).merge(z.object({ phenotype_id: z.string().trim().optional(), log10pvalue: z.string().trim().optional(), limit: z.number().optional(), verbose: z.enum(['true', 'false']).default('false') })))
  .output(z.array(variantPhenotypeFormat))

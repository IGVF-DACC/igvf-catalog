...
async function executeVariantsBiosamplesQuery (input: paramsFormatType, variantIds: string[] | undefined, biosampleIds: string[] | undefined): Promise<any[]> {
  ...
  const query = `
    ...
    RETURN {
      ...
      'inputCountRef': record.inputCountRef OR record.DNA_count_ref,
      'inputCountAlt': record.inputCountAlt OR record.DNA_count_alt,
      'outputCountRef': record.outputCountRef OR record.RNA_count_ref,
      'outputCountAlt': record.outputCountAlt OR record.RNA_count_alt,
